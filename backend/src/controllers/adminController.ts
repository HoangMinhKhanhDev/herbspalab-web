...
export const getStats = asyncHandler(async (req: Request, res: Response) => {
  ...
  const sourceMap: Record<string, number> = { 'Trực tiếp': 0, 'Google': 0, 'Facebook': 0, 'TikTok': 0, 'Zalo': 0, 'Khác': 0 };
  trafficSources.forEach(t => {
    const ref = (t.referrer || '').toLowerCase();
    if (!ref || ref === 'null' || ref === '') sourceMap['Trực tiếp'] = (sourceMap['Trực tiếp'] || 0) + 1;
    else if (ref.includes('google')) sourceMap['Google'] = (sourceMap['Google'] || 0) + 1;
    else if (ref.includes('facebook') || ref.includes('fb.')) sourceMap['Facebook'] = (sourceMap['Facebook'] || 0) + 1;
    else if (ref.includes('tiktok')) sourceMap['TikTok'] = (sourceMap['TikTok'] || 0) + 1;
    else if (ref.includes('zalo')) sourceMap['Zalo'] = (sourceMap['Zalo'] || 0) + 1;
    else sourceMap['Khác'] = (sourceMap['Khác'] || 0) + 1;
  });
...
export const getTrafficReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  logs.forEach(t => {
    const ref = (t.referrer || '').toLowerCase();
    if (!ref || ref === 'null') sourceMap['Trực tiếp'] = (sourceMap['Trực tiếp'] || 0) + 1;
    else if (ref.includes('google')) sourceMap['Google'] = (sourceMap['Google'] || 0) + 1;
    else if (ref.includes('facebook') || ref.includes('fb.')) sourceMap['Facebook'] = (sourceMap['Facebook'] || 0) + 1;
    else if (ref.includes('tiktok')) sourceMap['TikTok'] = (sourceMap['TikTok'] || 0) + 1;
    else if (ref.includes('zalo')) sourceMap['Zalo'] = (sourceMap['Zalo'] || 0) + 1;
  ...
  logs.forEach(t => {
    ...
    if (ua.includes('mobile') || ua.includes('android') || ua.includes('iphone')) deviceMap['Mobile'] = (deviceMap['Mobile'] || 0) + 1;
    else if (ua.includes('ipad') || ua.includes('tablet')) deviceMap['Tablet'] = (deviceMap['Tablet'] || 0) + 1;
...
export const getCustomerReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  customersWithOrders.forEach(c => {
    ...
    if (count === 0) segments['Mới (chưa mua)']++;
    else if (count === 1) segments['1 đơn hàng']++;
    else if (count <= 5) segments['2-5 đơn hàng']++;
    else segments['Trung thành (>5)']++;
  });
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const count = customersWithOrders.filter(c =>
